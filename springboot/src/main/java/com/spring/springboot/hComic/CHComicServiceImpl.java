...
import com.spring.springboot.hComicTag.CHComicTagMapper;

import java.util.*;

@Service
public class CHComicServiceImpl implements CHComicService
{
    @Value("${filesPath}") //获取application.properties中的全局变量
    private String filesPath;

    @Autowired
    CHComicMapper hComicMapper;
    @Autowired
    CHComicTagMapper hComicTagMapper;

    @Override
    public List<CHComic> getHComicList(CHComicRequestPojo hComicRequest)
    {
        ...
        List<CHComic> hComicList = hComicMapper.getHComicList(hComicRequest);
    ...
    {
        CHComic comic = hComicMapper.getHComicById(id);
        comic.setHComicTagList(hComicTagMapper.getHComicTagsByComicId(id));
        CHComicData data = new CHComicData();
    ...
    @Override
    public int getHComicCount(CHComicRequestPojo pojo)
    {
