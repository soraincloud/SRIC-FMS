...
import com.spring.springboot.hVideoTag.CHVideoTagMapper;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Service;
import static com.spring.springboot.tools.CImageToBase64.imageToBase64;

import java.util.List;

@Service
public class CHVideoServiceImpl implements CHVideoService
{
    @Value("${filesPath}") //获取application.properties中的全局变量
    private String filesPath;

    @Autowired
    CHVideoMapper hVideoMapper;
    @Autowired
    CHVideoTagMapper hVideoTagMapper;

    @Override
    public List<CHVideo> getHVideoList(CHVideoRequestPojo hVideoRequest)
    {
        ...
        List<CHVideo> hVideoList = hVideoMapper.getHVideoList(hVideoRequest);
    ...
    {
        CHVideo video = hVideoMapper.getHVideoById(id);
        video.setHVideoTagList(hVideoTagMapper.getHVideoTagsByVideoId(id));
        return video;
    }

    @Override
    public int getHVideoCount(CHVideoRequestPojo pojo)
    {
